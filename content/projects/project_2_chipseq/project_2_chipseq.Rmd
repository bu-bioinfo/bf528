...
installed. This style of having each task run in a dedicated, isolated
...
instruction at alo times: **Do not alter any file in the envs/ directory for any
reason at any time**

Here is the basic outline of the changes:

1. In your template repo, I have provided you with a `install_packages.sh` script
that will make a conda environment for project 2 that only contains `snakemake`
and a few python libraries including `pandas` and `jupyterlab`. Please run
this script, `bash install_packages.sh`, it will create a conda environment 
named `bf528_project2` with specific versions of these packages. You will still 
activate this environment whenever working on the project and the rest of the 
changes will be handled by snakemake. You may change the name of the created 
environment but do not alter anything else.

2. Also in your template repo, you can **LOOK** but do not alter any files in
...
parameters in an easy to parse format. You can see that they are named
descriptively with the names of major packages we have likely talked about.
...
we have listed our *dependencies*, which is simply a specific version of the
package(s) we want installed. It will look something like below:
...
1. Use the `install_packages.sh` script included in the repo to make a conda
...
2. Do not under any circumstances edit any of the files located in envs/ for 
reasons that will be explained below.

3. We have provided you with a skeleton of the snakefiles, which also lists
what you'll need to do for that week. Do not under any circumstances alter the
`conda` directive. 

4. Whenever you run snakemake (whether local with the `-c` flag or using `qsub`
...
--cluster-generic-submit-cmd "your qsub command here" --jobs X
```
...
5. The first time you run your workflow (locally or via qsub), when provided the
...
directive for each rule. This will happen once, and in subsequent runs, snakemake
will simply activate the now existing environment. 

6. I have provided you with templates containing just the rule names and the
appropriate `conda` directive for each rule. Do not change these. These snakefile
templates contain all of the rules you will need to complete for each week. 
...
want as we do not want to run into issues where we use more space than we have
on our partition. 
...
snakemake skeletons with all of the rules you'll need to complete for that week.

As we settled into a format, there has been some mixed communication on my part
about what to do each week. For this project, I have settled on a consistent
format that should make it more clear what is expected each week. Unless
otherwise specified, you should accomplish three main goals each week:

1. Complete your snakefile for that week and ensure it runs from start to
finish. Your dryrun should complete succesfully and the commands you run with
whatever tools we are utilizing should also complete succesfully. You may use
the provided subsampled data once again to quickly ensure your commands work on
actual data.

2. Using your knowledge of `qsub` and snakemake, run your completed and working
snakefile on the full data.

3. Write in the provided Rmarkdown notebooks a quick draft of the methods
...
[Week 1](#week-1-getting-started-chipseq)
