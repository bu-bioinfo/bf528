...
1. Note the full paths of the files located in:
/projectnb/bf528/materials/project-2-chipseq/subsampled_files/. Make a CSV
samplesheet that encodes the name of the sample, and the path to the actual
file:

```
name,path
IP_rep1,/projectnb/bf528/materials/project-2-chipseq/subsampled_files/IP_rep1.subset.fastq.gz
...
```

**Please note that the subsample_files are named differently than the full files!**

2. Encode the path to the human reference genome,associated GTF and the TruSeq3
adapters fasta and add these paths to your `nextflow.config` as params. An
example you can see below:

```
genome = "/projectnb/bf528/materials/project-2-chipseq/refs/GRCh38.primary_assembly.genome.fa.gz"
```
