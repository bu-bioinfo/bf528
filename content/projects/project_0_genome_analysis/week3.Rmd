...
environment specification with python installed. You can have 

The inputs be the same shape and structure as the `fa_ch`. You will need to
specify how you want the output file named. The shell/script portion will be the
command for executing the associated `.py` script and providing it the
appropriate inputs as specified by `argparse`. Please write the new file to the
`results/` directory using `publishDir`. Name this output file using the name
value passed in the channel (i.e. ${name}_genome_stats.txt)
