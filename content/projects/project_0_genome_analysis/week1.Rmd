...
so in the included `docs/week#_tasks.ipynb`.
:::
...
8. Answer all of the questions in the provided jupyter notebook (some won't be
...
3. From now on, you should always be exclusively be working in an interactive
...
To begin, we are going to use the module with miniconda installed on the SCC. We
...
recent version available for Nextflow in the provided jupyter notebook in the
`docs/` directory.
...
Your channel when observed using `view` should look something like:

[name_of_bacterial_genome, /path/to/reference/genome.fna]

This is the information that is passed to various nextflow processes that enable
nextflow to determine what processes to run on what files.
