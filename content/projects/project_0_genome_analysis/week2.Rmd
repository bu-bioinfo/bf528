...
information about the structure of the genome file that will enable far more
efficient searching than simply scanning from beginning to end for a target
sequence.
...
the file descriptively (e.g. prokka_env.yml)
...
share or re-use the modules we generate that run certain commands in specified
...
running. We will be running Prokka with mostly default parameters and your
command should include the exact options and no more: `--cpus`, `--outdir`,
...
nextflow can recognize the files it needs to generate. In your command, you have
specified the name of the output directory, use this same value as your output.
...
(and the input) as well as the newly generated index file.
