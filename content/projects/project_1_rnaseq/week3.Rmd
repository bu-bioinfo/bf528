...
the counts. The strategy used for pre-filtering counts will depend entirely on
...
all 8 samples have a non-zero value, it should contain 564 rows. 
  ...
  2. This script should take an unfiltered counts matrix as input, filter that 
    ...
    a new file.
    - You can copy the argparse arguments from the `parse_gtf.py` script and simply
    ...
    - You may use the file listed above to **test** that your code is working as
      ...
      with the file you generated previously**
...
- Generated a snakemake rule and a python script that filter the counts matrix
  to only retain genes (rows) where every sample has a non-zero count
