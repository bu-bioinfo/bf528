...
```
The important aspects in that command are the `--executor cluster-generic` command
which specifies which submit command to run. In our case, the SCC uses the SGE 
cluster software to submit, queue and schedule jobs from many different users. 
`qsub` is the primary command used by SGE to recognized jobs submitted by users
and the cluster-generic executor is snakemake's plugin to handle SGE commands. 
The `--cluster-generic-submit-cmd` is the appropriate qsub command to request
