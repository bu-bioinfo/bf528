...
`test.fastq.gz` which should run in a short amount of time. In parallel, you
should use qsub scripts in the style of previous projects to submit jobs
individually for the full data file.

The `test.fastq.gz` file is a subset of the original file containing only 1000
reads, and thus, the major bottleneck for run time will simply be the waiting
time for queued jobs on the SCC. It should run quickly enough that keeping an
active terminal window should not be too cumbersome. If you are already familiar
with tmux or screen, you may develop the workflow to operate on the full files
...
8. The authors of the paper used MEME-ChIP to perform motif finding using their 
   ...
   Please ensure that you set the <genome> argument as `/projectnb/bf528/project_3_chipseq/references/hg19/GRCh37.p13.genome.fa` and 
   include the additional argument `-size 200`. 
...
compared. We will then use the bigWig files to plot the signal coverage
