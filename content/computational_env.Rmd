...
Conda is a software package that provides package, dependency and environment 
management for all of the common platforms. Anaconda is a set of included packages
and miniconda. For our purposes, we will only be using miniconda. Miniconda will
allow you to create separate environments, each containing their own packages and
package dependencies. Miniconda will handle resolving all of the different 
versions and package dependencies needed for the requested tools. 

**When you might want to do this**
- Conda should be the default environment management tool used if better solutions
...
description into a YML file. For containers, we will strive to find up-to-date
containers.
...
samtools that we want installed. Conda will handle installation of any necessary
dependencies. We will keep this YML file contained within the repository in
which it is being used. This would allow us or others to exactly recreate the
...
file stored in your home directory (~/.condarc). It will change the location
where conda installs all of its packages to a directory specified. By default, it
would install these packages into your home directory which is only allotted
10gb.
